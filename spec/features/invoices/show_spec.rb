...
RSpec.describe 'invoices show' do
  before :each do
    ...
    #################
    @invoice_11 = Invoice.craete!(customer_id: @customer_1.id, status: 2)
    ...
    ###############
    @ii_20 = InvoiceItem.create!(invoice_id: @invoice_11.id, item_id: @item_1.id, quantity: 10, unit_price: 10, status: 2)
    @ii_21 = InvoiceItem.create!(invoice_id: @invoice_11.id, item_id: @item_2.id, quantity: 20, unit_price: 8, status: 2)
    @ii_22 = InvoiceItem.create!(invoice_id: @invoice_11.id, item_id: @item_1.id, quantity: 30, unit_price: 5, status: 2)
  ...
  xit 'shows the total revenue for the merchant without discounts' do
    visit merchant_invoice_path(@merchant1, @invoice_20)
  end

  xit 'shows the total discounted revenue for the merchant WITH discounts' do
    visit merchant_invoice_path(@merchant1, @invoice_20)
  end
end
